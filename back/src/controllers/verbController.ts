...
const VALID_POLARITIES: VerbPolarity[] = [VerbPolarity.AFFIRMATIVE, VerbPolarity.NEGATIVE];
...
export const getRandomVerb = (req: VerbReq, res: Response) => {
  ...
  const verb = generateRandomVerb(verbRepository, kinds, tenses, registers, polarities);
  res.json(verb);
};
...
const parsePolarities = (raw: string | undefined): VerbPolarity[] | undefined => {
  if (!raw) return undefined;
  const valid = raw
    .split(',')
    .map((p) => p.trim())
    .filter((p): p is VerbPolarity => VALID_POLARITIES.includes(p as VerbPolarity));
  return valid.length > 0 ? valid : undefined;
};
